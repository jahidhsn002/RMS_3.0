		...
		<table class="table white table-bordered">
			<thead>
				<tr>
					...
					<th data-priority="2">Payment</th>
					<th data-priority="4">Left</th>
					<th data-priority="8">Date</th>
					<th data-priority="9">Slip</th>
				</tr>
			</thead>
			<tbody>
				<?php foreach($history_sales as $food): ?>
				<tr>
					<td><?php echo $prefix . $food->id; ?></td>
					<td><?php echo $food->total . $price_unit; ?></td>
					<td><?php echo $food->discount . $price_unit; ?></td>
					<td><?php echo $food->paid . $price_unit; ?></td>
					<td><?php echo $food->payment . $price_unit; ?></td>
					<td><?php echo $food->left . $price_unit; ?></td>
					<td><?php echo unix_to_human($food->time, FALSE, 'us'); ?></td>
					<td><a class="btn btn-sm btn-primary" href="<?php echo site_url('salse/partsell_slip/' . $food->id); ?>"><span class="glyphicon glyphicon-credit-card"></span></a></td>
				</tr>
				<?php endforeach; ?>
			</tbody>
		</table>
